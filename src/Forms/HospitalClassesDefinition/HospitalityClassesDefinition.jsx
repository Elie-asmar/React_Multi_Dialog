...
// Still need to adjust api calls to check if code is available or not 
export function HospitalityClassesDefinition({ uuid, useInModal, routing, ...props }) {
    ...
    useEffect(() => {
        ...
        if (!useInModal) {
            ...
            if (location.pathname.toLocaleLowerCase().includes("/edit")) {
                if (priv?.Modify === 0) {
                    navigate('/HospitalDefinition', { replace: true })
                }
                else {
    ...
    const FillData = useCallback(async (id) => {
        ...
        if (id) {
            ...
            let data = await FetchData(`${urlPath}/Classes/getMainClassesbyId`, 'get', _data);
            setisLoading(prv => prv - 1)
            if (data && data.data.length > 0) {
                let o = {
                    code: data.data[0].cls_Code ? data.data[0].cls_Code : '',
                    status: data.data[0].cls_Status,
                    desc: data.data[0].cls_Desc,
                    abrvDesc: data.data[0].cls_AbvDesc ? data.data[0].cls_AbvDesc : '',
                    createdBy: '',
                    creationDate: '',
                    modifiedBy: '',
                    modifiedDate: '',
    ...
    const isCodeAvailable = useCallback(async (id) => {
        ...
        let data = await FetchData(`${urlPath}/Classes/getMainClasses`, 'get', null,
            id ? (e) => { return e.cls_Code.toString() === id } : () => false);
    ...
    const handleSave = useCallback(async () => {
        ...
        if (!isEmpty(missing)) {
            ...
            let objToSave = {
                Cls_Code : state.code,
                Cls_Desc: state.desc,
                Cls_AbvDesc: state.abrvDesc,
                Cls_Status: state.status,
                Cls_Dstmp: null,
                Cls_Ustmp: null,
            
            }
            const response = await FetchData(`${urlPath}/Classes/upsertMainClasses`, 'post', objToSave).
                catch(error => {
                    throw error
                })
            if (response.success) {
    ...
    return (
        <>
            ...
            <div className="animated fadeIn activeComponent" >
                ...
                />
                <div className="row">
                    <div className="col-md-12 title">Hospital Class Definition  - {state.mode} </div>
                </div>
