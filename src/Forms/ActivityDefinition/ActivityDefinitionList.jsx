...
export function ActivityDefinitionList({ uuid, ...props }) {
    ...
    const [data, error, refetch] = useFetchData(`${urlPath}/ActivityDef/getMainActivity`, 'get');
    //#region "Side Effects"
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        if (data) {
            ...
            setState(prv => {
                return { ...prv, ActivityList: data.map((e) => { return { ...e, 'Status': e.act_Sts.toLowerCase() === 'a' ? 'Active' : 'Inactive' } }) }
            })
    ...
    const editActivity = useCallback((id) => {
        navigate('/ActivityDefinition/edit', { state: { editid: id, uuid } })
    }, []);
    
    const deleteActivity = useCallback(async(id) => {
        const response = await FetchData(`${urlPath}/ActivityDef/deleteMainActivity`, 'get', {Act_Code: id}).
            catch(error => {
                throw error
            })
            refetch();
    ...
    const addActivity = useCallback(() => {
        navigate('/ActivityDefinition/add', { state: { uuid } })
    }, [])
                    ...
                    if (state.privileges.Modify === 1) {
                        HTML.push(
                            <RoundedIcon
                                ...
                                onClick={() => { editActivity(row.original.act_Code) }} /* This is the onClick function */
                                tooltip={"Edit"}
                    ...
                    if (true) {
                        HTML.push(
                            <RoundedIcon
                                ...
                                position="right" /* Other options are left and center  */
                                onClick={() => { deleteActivity(row.original.act_Code) }} /* This is the onClick function */
                                tooltip={"Delete"}
    ...
    return (
        <div className="animated fadeIn">
            ...
            <AdvancedTable
                dataArray={state.privileges.View === 1 || state.privileges.Modify === 1 ? state.ActivityList : []}
                columns={columns}
                AdvancedTableHeight={CalculateTitleHeight("ActivityListContainer")}
                enableSearch
