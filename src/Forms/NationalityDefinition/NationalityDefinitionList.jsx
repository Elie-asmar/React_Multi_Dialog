...
export function NationalityDefinitionList({ uuid, ...props }) {
    ...
    // const [datajson, errorjson] = useFetchData('DataFiles/Forms/Nationality.json', 'get');
    const [data, error] = useFetchData(`${urlPath}/Nationality/getNationality`, 'get');
    //#region "Side Effects"
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        if (data) {
            setState(prv => {
                return { ...prv, NationalityList: data.map((e) => { return { ...e, 'nat_Status': e.nat_Status.toLowerCase() === 'a' ? 'Active' : 'Inactive' } }) }
            })
        }
        else if (error) {
        }
    ...
    const editNationality = useCallback((id) => {
        navigate('/Nationality/edit', { state: { editid: id, uuid } })
    }, []);

    const addNationality = useCallback(() => {
        navigate('/Nationality/add', { state: { uuid } })
    }, [])
                    ...
                    if (state.privileges.Modify === 1) {
                        HTML.push(
                            <RoundedIcon
                                ...
                                onClick={() => { editNationality(row.original.nat_Code) }} /* This is the onClick function */
                                tooltip={"Edit"}
    ...
    return (
        <div className="animated fadeIn">
            ...
            <AdvancedTable
                dataArray={state.privileges.View === 1 || state.privileges.Modify === 1 ? state.NationalityList : []}
                columns={columns}
