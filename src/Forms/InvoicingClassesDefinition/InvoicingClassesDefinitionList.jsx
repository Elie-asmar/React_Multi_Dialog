...
export function InvoicingClassesDefinitionList({ uuid, ...props }) {

    const STATE = {
        InvoicingClassesList: [],
        modal: false,
        privileges: {},
    ...
    const [data, error] = useFetchData(`${urlPath}/InvoicingClasses/getInvoicingClasses`, 'get');
    //#invoiceClass "Side Effects"
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        if (data) {
            ...
            setState(prv => {
                return { ...prv, InvoicingClassesList: data.map((e) => { return { ...e, 'Status': e.inc_Status.toLowerCase() === 'a' ? 'Active' : 'Inactive' } }) }
            })
    ...
    //#invoiceClass Memoized CallBacks
    ...
    const editInvoiceClass = useCallback((id) => {
        navigate('/InvoicingClassDef/edit', { state: { editid: id, uuid } })
    }, []);

    const addInvoiceClass = useCallback(() => {
        navigate('/InvoicingClassDef/add', { state: { uuid } })
    }, [])
    ...
    const drawTopActions = useCallback(() => {
        ...
        if (state.privileges.Add === 1) {
            HTML.push(<RoundedIcon
                ...
                onClick={addInvoiceClass}
                position="left"
                tooltip={"Add Invoice Class"}
                id={"add"}
            />);
        }
        ...
        return [
            {
                ...
                Cell: row => (
                    <div>
                        <span className="mobile-label">Class Code</span>
            ...
            },
            {
                ...
                Cell: row => {
                    return (
                        <div>
                            <span className="mobile-label">Class Description</span>
                    ...
                    if (state.privileges.Modify === 1) {
                        HTML.push(
                            <RoundedIcon
                                ...
                                onClick={() => { editInvoiceClass(row.original.inc_Code) }} /* This is the onClick function */
                                tooltip={"Edit"}
    ...
    return (
        <div className="animated fadeIn">
            <div id="InvoicingClassesListContainer" >
                {/* <Notification
            ...
            <AdvancedTable
                dataArray={state.privileges.View === 1 || state.privileges.Modify === 1 ? state.InvoicingClassesList : []}
                columns={columns}
                AdvancedTableHeight={CalculateTitleHeight("InvoicingClassesListContainer")}
                enableSearch
