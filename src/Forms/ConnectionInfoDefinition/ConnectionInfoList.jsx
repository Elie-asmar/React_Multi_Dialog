...
export function ConnectionInfoList({ uuid, ...props }) {
    ...
    const [data, error] = useFetchData('DataFiles/Forms/ConnectionInfoDefinition/ConnectionInfoDefinition.json', 'get');
    //#region "Side Effects"
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        if (data) {
            ...
            setState(prv => {
                return { ...prv, ConnectionInfoList: data.map((e) => { return { ...e, 'Status': 'Active' } }) }
            })
    ...
    const editNationality = useCallback((id) => {
        navigate('/ConnInfoDef/edit', { state: { editid: id, uuid } })
    }, []);

    const addNationality = useCallback(() => {
        navigate('/ConnInfoDef/add', { state: { uuid } })
    }, [])
    ...
    const columns = useMemo(() => {
        ...
        return [
            ...
            },
            {
                ...
                Cell: row => (
                    <div>
                        <span className="mobile-label">Database Name</span>
                        <span className="cell-value">{row.value}</span>
                    </div>
                ),
                width: screenWidth * 0.2
            ...
            {
                Header: 'Database Access Path',
                accessor: 'Con_Path',
                Cell: row => (
                    <div>
                        <span className="mobile-label">Database Access Path</span>
                        <span className="cell-value">{row.value}</span>
                    </div>
                ),
                width: screenWidth * 0.2
            ...
            },
            {
                ...
                Cell: row => (
                    <div>
                        <span className="mobile-label">Authentication Type</span>
                        <span className="cell-value">{row.value === 'W' ? 'Windows' : 'Other'}</span>
                    </div>
                ),
                width: screenWidth * 0.1
            ...
            },
            {
                ...
                Cell: row => {
                    ...
                    if (state.privileges.Modify === 1) {
                        ...
                        HTML.push(
                            <RoundedIcon
                                key={`btnEdit-${row.original.Con_DBCode}`}
                                ...
                                onClick={() => { editNationality(row.original.Con_DBCode) }} /* This is the onClick function */
                                tooltip={"Edit"}
    ...
    return (
        <div className="animated fadeIn">
            ...
            <AdvancedTable
                dataArray={state.privileges.View === 1 || state.privileges.Modify === 1 ? state.ConnectionInfoList : []}
                columns={columns}
                AdvancedTableHeight={CalculateTitleHeight("ConnectionListContainer")}
                enableSearch
