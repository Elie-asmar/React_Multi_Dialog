...
export function ProfessionDefinitionList({ uuid, ...props }) {
    // debugger
    ...
    const [data, error] = useFetchData('DataFiles/Forms/Professions.json', 'get');
    //#region "Side Effects"
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        if (data) {
            ...
            setState(prv => {
                return { ...prv, ProfessionList: data.map((e) => { return { ...e, 'Status': e.Pro_Status.toLowerCase() === 'a' ? 'Active' : 'Inactive' } }) }
            })
    ...
    const editProfession = useCallback((id) => {

        navigate('/ProfessionDef/edit', { state: { editid: id, uuid } })
    }, []);

    const addProfession = useCallback(() => {
        navigate('/ProfessionDef/add', { state: { uuid } })
    }, [])
                    ...
                    if (state.privileges.Modify === 1) {
                        HTML.push(
                            <RoundedIcon
                                ...
                                onClick={() => { editProfession(row.original.Pro_Code) }} /* This is the onClick function */
                                tooltip={"Edit"}
    ...
    return (
        <div className="animated fadeIn">
            <div id="ProfessionListContainer" >
                {/* <Notification
            ...
            <AdvancedTable
                dataArray={state.privileges.View === 1 || state.privileges.Modify === 1 ? state.ProfessionList : []}
                columns={columns}
                AdvancedTableHeight={CalculateTitleHeight("ProfessionListContainer")}
                enableSearch
