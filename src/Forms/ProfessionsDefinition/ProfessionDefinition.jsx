...
export function ProfessionDefinition({ uuid, useInModal, routing, ...props }) {
    ...
    useEffect(() => {
        ...
        if (!useInModal) {
            ...
            if (location.pathname.toLocaleLowerCase().includes("/edit")) {
                if (priv?.Modify === 0) {
                    navigate('/ProfessionDef', { replace: true })
                }
                else {
    ...
    useEffect(() => {
        ...
        let dropDownOptions = [];
        FetchData('DataFiles/Forms/Professions.json', 'get', null,
    ...
    const FillData = useCallback(async (id) => {
        if (id) {
            ...
            let data = await FetchData('DataFiles/Forms/Professions.json', 'get', null,
                (e) => { return e.Pro_Code.toString() === id.toString() });

            setisLoading(prv => prv - 1)
            if (data && data.length > 0) {
                let o = {
                    code: data[0].Pro_Code ? data[0].Pro_Code.toString() : '',
                    desc: data[0].Pro_Desc ? data[0].Pro_Desc : '',
                    status: data[0].Pro_Status,
                    createdBy: '',
                    creationDate: '',
                }
                setState(prv => {
    ...
    const isCodeAvailable = useCallback(async (id) => {
        ...
        let data = await FetchData('DataFiles/Forms/Professions.json', 'get', null,
            id ? (e) => { return e.Pro_Code.toString() === id } : () => false);
    ...
    return (
        <>
            ...
            <div className="animated fadeIn activeComponent" >
                ...
                />
                <div className="row">
                    <div className="col-md-12 title">Professions Definition  - {state.mode} </div>
                </div>
