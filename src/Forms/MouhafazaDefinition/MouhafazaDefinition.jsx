...
export function MouhafazaDefinition({ uuid, useInModal, routing, ...props }) {
    ...
    useEffect(() => {
        ...
        if (!useInModal) {
            ...
            if (location.pathname.toLocaleLowerCase().includes("/edit")) {
                if (priv?.Modify === 0) {
                    navigate('/MouhafazaDef', { replace: true })
                }
                else {
    ...
    const FillData = useCallback(async (id) => {
        if (id) {
            ...
            let data = await FetchData('DataFiles/Forms/MouhafazaDefinition.json', 'get', null,
                (e) => { return e.Moh_Code.toString() === id.toString() });
            setisLoading(prv => prv - 1)
            if (data && data.length > 0) {

                let o = {
                    code: data[0].Moh_Code ? data[0].Moh_Code.toString() : '',
                    desc: data[0].Moh_Description ? data[0].Moh_Description : '',
                    arabicdesc: data[0].Moh_ArDesc ? data[0].Moh_ArDesc : '',
                    status: data[0].Moh_Status,
                    createdBy: '',
                    creationDate: '',
                }
                setState(prv => {
    ...
    const isCodeAvailable = useCallback(async (id) => {
        ...
        let data = await FetchData('DataFiles/Forms/MouhafazaDefinition.json', 'get', null,
            id ? (e) => { return e.Moh_Code.toString() === id } : () => false);
    ...
    return (
        <>
            ...
            <div className="animated fadeIn activeComponent" >
                ...
                />
                <div className="row">
                    <div className="col-md-12 title">Mouhafaza Definition  - {state.mode} </div>
                </div>
