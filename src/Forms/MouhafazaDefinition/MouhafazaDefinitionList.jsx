...
export function MouhafazaDefinitionList({ uuid, ...props }) {
    // debugger
    ...
    const [data, error] = useFetchData('DataFiles/Forms/MouhafazaDefinition.json', 'get');
    //#region "Side Effects"
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        if (data) {
            ...
            setState(prv => {
                return { ...prv, MouhafazaList: data.map((e) => { return { ...e, 'Status': e.Moh_Status.toLowerCase() === 'a' ? 'Active' : 'Inactive' } }) }
            })
    ...
    const editMouhafaza = useCallback((id) => {
        navigate('/MouhafazaDef/edit', { state: { editid: id, uuid } })
    }, []);

    const addMouhafaza = useCallback(() => {
        navigate('/MouhafazaDef/add', { state: { uuid } })
    }, [])
    ...
    const drawTopActions = useCallback(() => {
        ...
        if (state.privileges.Add === 1) {
            HTML.push(<RoundedIcon
                ...
                onClick={addMouhafaza}
                position="left"
                tooltip={"Add Mouhafaza"}
                id={"add"}
            />);
        }
                    ...
                    if (state.privileges.Modify === 1) {
                        HTML.push(
                            <RoundedIcon
                                ...
                                onClick={() => { editMouhafaza(row.original.Moh_Code) }} /* This is the onClick function */
                                tooltip={"Edit"}
    ...
    return (
        <div className="animated fadeIn">
            ...
            <AdvancedTable
                dataArray={state.privileges.View === 1 || state.privileges.Modify === 1 ? state.MouhafazaList : []}
                columns={columns}
                AdvancedTableHeight={CalculateTitleHeight("MouhafazaListContainer")}
                enableSearch
